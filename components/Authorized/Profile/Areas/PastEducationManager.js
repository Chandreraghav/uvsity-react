...
import { isEmptyObject, isValidDate, isValidDatePeriod } from "../../../../utils/utility";
...
function PastEducationManager(props) {
  ...
  const handleSubmit=()=>{
    ...
    if (props.event) {
      ...
      if(!fromDate){
        setError('fromDate','Please specify the year and month you started this course.')
        hasErrors=true
      }
      if(!toDate){
        setError('toDate','Please specify the year and month you completed this course.')
        
        hasErrors=true
      }
      if(!educationInstitution){
        setError('educationInstitution','Please enter name of an educational institution e.g University/College/School')
        hasErrors=true
      }
      if(!degree){
        setError('degree','Please enter a educational qualification, degree or diploma, e.g MSc, PH.d, BS, MS, High School, Middle School etc')
        
        hasErrors=true
      }
      if(!campus){
        setError('campus','Please enter the campus location from where you obtained this qualification, e.g Boston, New York, Delhi etc')
        hasErrors=true
      }
  ...
  return (
    <div>
      <Typography
        className=" font-semibold py-3 dark:text-gray-600 text-gray-dark"
        variant="subtitle2"
      >
        Past Education
      </Typography>

      <Box className={`${isUpdating ? "control__disabled__opaque" : ""} `} sx={{ width: "100%" }}>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker
                ...
                onChange={(newvalue) => {
                  ...
                  if (isValidDate(newvalue, "MMMM,yyyy")) clearErrors("toDate");
                }}
              />
            </LocalizationProvider>
          </Grid>
